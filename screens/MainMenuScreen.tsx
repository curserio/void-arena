...
import { GameDifficulty, PersistentData } from '../types';
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950/80 backdrop-blur-lg z-[200]">
            ...
            <div className="flex gap-2 mb-6 p-2 bg-slate-900/60 rounded-xl border border-slate-800 overflow-x-auto max-w-full">
                {Object.values(DIFFICULTY_CONFIGS).map(diff => {
                    const isUnlocked = (persistentData.currentLevel || 1) >= diff.minRank;
            ...
            <div className="flex flex-col gap-4 w-full max-w-xs mt-2">
                <button onClick={onStartStandard} className="py-6 bg-cyan-500 text-slate-950 font-black text-2xl rounded-2xl active:scale-95 transition-all shadow-lg shadow-cyan-500/20">START MISSION</button>

                <div className="grid grid-cols-2 gap-4">
                    <button onClick={onOpenGarage} className="py-4 bg-slate-800 text-white font-black text-xl rounded-2xl border border-slate-700 active:scale-95 transition-all">GARAGE</button>
                    <button onClick={onOpenDebug} className="py-4 bg-slate-900 text-emerald-400 font-black text-xl rounded-2xl border border-emerald-500/30 active:scale-95 transition-all flex items-center justify-center gap-2 shadow-inner hover:bg-slate-800">
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <button onClick={onOpenGuide} className="py-3 bg-slate-900 text-slate-400 font-bold text-sm rounded-xl border border-slate-800 active:scale-95 transition-all flex items-center justify-center gap-2">
                        <i className="fa-solid fa-book" /> MANUAL
                    </button>
                </div>

                <button onClick={onOpenCheats} className="py-2 bg-transparent text-red-900/50 font-bold text-xs uppercase tracking-widest hover:text-red-500 hover:bg-slate-900 rounded-lg transition-all">
