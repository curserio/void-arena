...
import { ENEMY_DEFINITIONS } from '../data/enemies/definitions';
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  ...
                  if (typeof def.color === 'string') {
                    ...
                    // Generate a representative color
                    color = `hsl(${def.color.hueMin}, ${def.color.saturation}%, ${def.color.lightness}%)`;
                  }
                  ...
                  return (
                    <EnemyCard
                      ...
                      name={def.name}
                      role={def.role}
                      baseHp={def.baseHealth * viewLevel}
                      speed={type === EnemyType.STRIKER ? "V. Fast" : type === EnemyType.KAMIKAZE ? "Extreme" : type === EnemyType.BOSS_DESTROYER ? "Very Slow" : "Normal"} // Speed label is subjective, keeping simple logic or we could add 'speedLabel' to definitions later
